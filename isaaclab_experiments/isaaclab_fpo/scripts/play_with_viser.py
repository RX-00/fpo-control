...
Example usage:
    # First extract task-specific assets
...
def resolve_asset_dir(task: str, requested_asset_dir: str | None) -> Path:
    """Resolve a Viser asset directory, including task aliases."""
    if requested_asset_dir:
        return Path(requested_asset_dir)

    asset_root = Path(__file__).resolve().parents[1] / "viser_assets"
    task_clean = task.lower().replace(":", "_").replace("-", "_")
    aliases = {
        # Fallback for older checkouts where the tracking-specific bundle has
        # not been generated by isaac_asset_extractor.py yet.
        "tracking_flat_g1_v0": ["isaac_velocity_flat_g1_v0"],
    }
    candidates = [asset_root / task_clean]
    candidates.extend(asset_root / name for name in aliases.get(task_clean, []))

    for candidate in candidates:
        if candidate.exists():
            if candidate.name != task_clean:
                print(f"[INFO] Using Viser asset alias for {task}: {candidate}")
            return candidate

    candidate_list = "\n".join(f"  - {candidate}" for candidate in candidates)
    raise FileNotFoundError(
        "No Viser asset directory found. Checked:\n"
        f"{candidate_list}\n"
        "Pass --asset-dir to use a custom extracted asset directory."
    )
...
def main():
    ...
    if args_cli.viser:
        ...
        try:
            asset_dir = resolve_asset_dir(args_cli.task, args_cli.asset_dir)
    ...
    while simulation_app.is_running():
        ...
        if viser_viz is not None:
            ...
            try:
                ...
                viser_viz.update_from_env(
                    base_env,
                    velocity_commands=not task_name.startswith("Tracking-"),
                    rewards=rewards,
                    actions=actions,
                )
            except Exception as e:
